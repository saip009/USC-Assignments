...
def regularized_linear_regression(X, y, lambd):
  ...
  Xt = X.transpose()
  XtX = np.dot(Xt, X)
  XtXr = XtX + lambd * np.identity(X.shape[1])
  w = np.linalg.inv(XtXr).dot(Xt).dot(y)
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
  ...
  for lambd in lambds:
      ...
      mses.append(test_error(w, Xval, yval))

  bestlambda = lambds[np.argsort(np.array(mses))[0]]
