...
def transform_image(image, code_vectors):
    ...
    # print(image.shape, data.shape, code_vectors.shape)
    x = image.shape[0] * image.shape[1]
    y = image.shape[2]
    data = image.reshape(x, y)
    temp42002 = ((code_vectors - np.expand_dims(data, axis=1)) ** 2)
    dist_mat = np.sum(temp42002, axis=2)
    r = np.argmin(dist_mat, axis=1)
    # m, r, it = KMeans.fit(data)
